...
Analyze this data and create a comprehensive dashboard report following the exact JSON structure specified."""
        ...
        try:
            ...
            logger.error(f"Gemini aggregation failed: {gemini_error}")
            logger.info("Using fallback aggregation with basic scores...")
            
            # Create a basic aggregated result from the raw data
            # Create a basic aggregated result from the raw data
            aggregated_result = {
                "overall_score": 7.0,
                ...
                "category_scores": {
                    "visual_presentation": {
                        "score": visual_data.get('overall_visual_score', 70),
                        "body_language": 70,
                        "posture": visual_data.get('posture_score', 70),
                        "gestures": visual_data.get('gesture_score', 70),
                        "eye_contact": visual_data.get('eye_contact_score', 70),
                        "facial_expressions": 70,
                        "professional_appearance": 70
                    },
                    "vocal_delivery": {
                        "score": audio_data.get('articulation_score', 60),
                        "pace": 70,
                        "pace_wpm": audio_data.get('words_per_minute', 0),
                        "optimal_pace_range": "140-160 wpm",
                        "articulation": audio_data.get('articulation_score', 60),
                        "filler_word_frequency": audio_data.get('filler_words_count', 0),
                        "pause_effectiveness": 70,
                        "volume_consistency": 70
                    },
                    "content": {
                        "score": content_data.get('overall_content_score', 70),
                        "structure": content_data.get('structure_score', 70),
                        "clarity": content_data.get('clarity_score', 70),
                        "engagement": content_data.get('engagement_score', 70),
                        "depth": content_data.get('content_depth_score', 70)
                    },
                    "tone_emotion": {
                        "score": 70,
                        "dominant_emotion": audio_data.get('dominant_emotion', 'neutral'),
                        "emotion_appropriateness": 70,
                        "confidence_level": 70,
                        "authenticity": 70,
                ...
                "key_metrics": {
                    "speech_duration_seconds": audio_data.get('speech_duration_seconds', 0),
                    "word_count": audio_data.get('word_count', 0),
                    "words_per_minute": audio_data.get('words_per_minute', 0),
                    "filler_words_count": audio_data.get('filler_words_count', 0),
                    "filler_words_per_minute": 0,
                    "gesture_frequency": 0,
                    "nervous_habits_count": 0
                },
                "strengths": [
                    {
                        "category": "delivery",
                        "strength": f"Speaking pace: {audio_data.get('speaking_pace', 'unknown')}",
                        "evidence": f"{audio_data.get('words_per_minute', 0)} WPM"
                    },
                    {
                        "category": "visual",
                        "strength": "Visual analysis completed",
                        "evidence": f"Overall visual score: {visual_data.get('overall_visual_score', 0)}/10"
                    },
                    {
                        "category": "content",
                        "strength": "Content analysis completed",
                        "evidence": f"Content score: {content_data.get('overall_content_score', 0)}/10"
                    }
                ],
                "improvements": [
                    {
                        "category": "general",
                        "issue": "Using simplified analysis",
                        "current_metric": "Fallback mode active",
                        "target_metric": "Full AI analysis",
                        "recommendation": "Wait for API rate limits to reset for detailed feedback",
                        "priority": "low",
                        "estimated_effort": "low"
                    },
                    {
                        "category": "delivery",
                        "issue": f"Filler words detected: {audio_data.get('filler_words_count', 0)}",
                        "current_metric": f"{audio_data.get('filler_words_count', 0)} fillers",
                        "target_metric": "Minimal filler words",
                        "recommendation": "Practice reducing filler words like 'um', 'uh', 'like'",
                        "priority": "medium",
                        "estimated_effort": "medium"
                    }
                ],
                "next_steps": [
                    "Review detailed analysis when available",
                    "Focus on maintaining consistent speaking pace",
                    "Practice reducing filler words"
                ],
                "summary": "Analysis completed using fallback mode due to API constraints.",
                "note": "Full AI aggregation unavailable - using compiled scores from pipeline"
            }
            logger.info("✅ Fallback aggregation created successfully")
...
def save_results(aggregated_results: Dict, video_id: str) -> str:
    """
...
def generate_summary_stats(aggregated_results: Dict) -> Dict:
    """
    Generate quick summary statistics for logging/monitoring
    
    Args:
        aggregated_results: Complete aggregation results
