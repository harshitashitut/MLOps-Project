...
// UPDATED VERSION - With status polling and progress tracking

import React, { useState } from "react";
import { Upload, Mic, ArrowLeft, CheckCircle2, AlertCircle, Loader2, TrendingUp, Award, Target } from "lucide-react";
import { analyzeVideoComplete } from "../api/videoAnalysis";
...
export default function PublicSpeaking() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await analyzeVideoComplete(file, (progressUpdate) => {
      ...
      // Backend returns the full structure, extract results
      setResults(response);
        ...
        {!results ? (
          <>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-black border border-green-500/20 rounded-2xl p-8 shadow-xl max-w-4xl mx-auto">
              ...
              {/* Progress Bar */}
              {uploading && (
                <div className="mt-6 p-6 bg-gray-800/50 rounded-xl border border-green-500/20">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {Object.entries(results.results.category_scores).map(([category, data]) => (
            ...
            <div className="bg-gradient-to-br from-gray-900 to-black border border-green-500/20 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{results.results.key_metrics.words_per_minute} WPM</p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Duration</p>
                  <p className="text-2xl font-bold text-white">{results.results.key_metrics.speech_duration_seconds.toFixed(1)}s</p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Filler Words</p>
                  <p className="text-2xl font-bold text-white">{results.results.key_metrics.filler_words_count}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-black border border-green-500/20 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {results.results.improvements.map((improvement, idx) => (
                  <div key={idx} className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    ...
                    <p className="text-sm text-gray-400">{improvement.recommendation}</p>
                  </div>
                ))}
              </div>
            </div>
